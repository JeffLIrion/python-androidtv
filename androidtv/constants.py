...
#: Get the audio state
CMD_AUDIO_STATE = r"dumpsys audio | grep paused | grep -qv 'Buffer Queue' && echo -e '1\c' " \
...
# to ``CURRENT_APP`` variable (assumes dumpsys output is momentarily set to ``CURRENT_APP`` variable)
CMD_PARSE_CURRENT_APP = "CURRENT_APP=${CURRENT_APP#*ActivityRecord{* * } " \
                        "&& CURRENT_APP=${CURRENT_APP#*{* * } " \
                        "&& CURRENT_APP=${CURRENT_APP%%/*} " \
                        "&& CURRENT_APP=${CURRENT_APP%\\}*}"

#: Assign focused application identifier to ``CURRENT_APP`` variable
CMD_DEFINE_CURRENT_APP_VARIABLE = "CURRENT_APP=$(dumpsys window windows | grep -E 'mCurrentFocus|mFocusedApp')"


#: Get the HDMI input
CMD_HDMI_INPUT = "dumpsys activity starter " \
                 "| grep -E -o '(ExternalTv|HDMI)InputService/HW[0-9]' -m 1 " \
                 "| grep -o 'HW[0-9]'"
...
CMD_MEDIA_SESSION_STATE = "dumpsys media_session " \
                          "| grep -A 100 'Sessions Stack' " \
                          "| grep -A 100 $CURRENT_APP " \
                          "| grep -m 1 'state=PlaybackState {'"
...
#: Define the list of commands used to get status
CMD_STATUS_LIST = [
    "screen_on",
    "awake",
    "audio_state",
...
#: Define the standard commands used to update AndroidTV
CMD_DEFAULTS = {
    "screen_on": CMD_SCREEN_ON,
    "awake": CMD_AWAKE,
    "audio_state": CMD_AUDIO_STATE,
    "wake_lock_size": CMD_WAKE_LOCK_SIZE,
    "parse_current_app": CMD_PARSE_CURRENT_APP,
    "define_current_app_var": CMD_DEFINE_CURRENT_APP_VARIABLE,
    "media_session_state": CMD_MEDIA_SESSION_STATE,
    "hdmi_input": CMD_HDMI_INPUT,
    "stream_music": CMD_STREAM_MUSIC,
    "running_apps": CMD_RUNNING_APPS_ANDROIDTV,
}
...
# (used by :func:`~androidtv.basetv.state_detection_rules_validator`)
VALID_STATE_PROPERTIES = ("audio_state", "media_session_state")

#: Properties that can be checked for custom state detection
# (used by :func:`~androidtv.basetv.state_detection_rules_validator`)
VALID_PROPERTIES = VALID_STATE_PROPERTIES + ("wake_lock_size",)

#: The required type for each entry in :py:const:`VALID_PROPERTIES`
# (used by :func:`~androidtv.basetv.state_detection_rules_validator`)
...
#: Default authentication timeout (in s) for :meth:`adb_shell.handle.tcp_handle.TcpHandle.connect`
# and :meth:`adb_shell.handle.tcp_handle_async.TcpHandleAsync.connect`
DEFAULT_AUTH_TIMEOUT_S = 10.0

#: Default timeout (in s) for :class:`adb_shell.handle.tcp_handle.TcpHandle`
# and :class:`adb_shell.handle.tcp_handle_async.TcpHandleAsync`
