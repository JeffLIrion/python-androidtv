...
CMD_CURRENT_APP_FULL = "dumpsys window windows | grep mCurrentFocus | sed -n 's/.*Window{[^ ]* [^ ]* //p' | sed -n -e 's/\/.*//p'"
CMD_MEDIA_SESSION_STATE = "dumpsys media_session | sed -n -e '/Sessions Stack/,$p' | sed -n -e \"/$(%s)/,$p\" | sed -e '/^\s*$/,$d' | grep -m 1 'state=PlaybackState {'" % CMD_CURRENT_APP_FULL
